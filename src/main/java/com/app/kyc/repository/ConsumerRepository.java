...
public interface ConsumerRepository extends JpaRepository<Consumer, Long>
{
   ...
   @Query(value = "select count(*) from consumers where consumer_status = :consumer_status and service_provider_id in (" + "select id from service_providers where industry_id = :industryId) and created_on > :start and created_on <= :end", nativeQuery = true)
   ...
   @Query(value = "select * from consumers where consumer_status = :consumer_status and id in (" + "select distinct consumer_id from consumers_services where service_id in " + "(select id from services where service_type_id = :serviceTypeId)) " + "and created_on > :start and created_on <= :end", nativeQuery = true)
   ...
   @Query(value = "select * from consumers where consumer_status = :consumer_status and id in (" + "select distinct consumer_id from consumers_services where service_id in " + "(select id from services where service_type_id = :serviceTypeId)) " + "and service_provider_id = :serviceProviderId " + "and created_on > :start and created_on <= :end", nativeQuery = true)
   List<Consumer> findAllConsumersByServiceProviderIdAndServiceTypeIdAndCreatedOnGreaterThanAndCreatedOnLessThanEqual(@Param("consumer_status")int consumer_status,Long serviceProviderId, Long serviceTypeId, Date start,
      Date end);

   List<Consumer> findAllConsumersByServiceProviderIdAndConsumerStatus(Long serviceProviderId, int consumer_status);

   @Query(value = "select * from consumers where consumer_status = :consumer_status and service_provider_id in (" + "select service_provider_id from services where id = :serviceId and created_by = :userId)", nativeQuery = true)
   List<Consumer> getAllByServiceIdAndUserId(@Param("consumer_status")int consumer_status, Long userId, Long serviceId);

  /* @Query(value = "select (select name from service_providers where id = service_provider_id) as name, count(*) as value, service_provider_id as serviceproviderid from consumers" + " where service_provider_id in :serviceProvidersIdList" + " and registration_date between :start and :end and consumer_status=0" + " group by name, serviceproviderid", nativeQuery = true)
   List<DashboardObjectInterface> getAndCountConsumersGroupedByServiceProviderId(@Param("serviceProvidersIdList") Collection<Long> serviceProvidersIdList, Date start, Date end);
*/
  @Query(value = "SELECT * FROM consumers", nativeQuery = true)
  List<DashboardObjectInterface> getAllCustomers();

   @Query(value = "select (select name from service_providers where id = service_provider_id) as name, count(distinct consumers.id) as value, service_provider_id as serviceproviderid from consumers" + " where service_provider_id in :serviceProvidersIdList" + " and registration_date between :start and :end and consumer_status=0" + " group by name, serviceproviderid", nativeQuery = true)
...
//    @Query("select CONCAT(CAST(YEAR(c.registrationDate) as string ),'-',CAST(MONTH(c.registrationDate) as string )) as name, CAST(count(distinct(c.id)) as int) as value from (select distinct(concat(cc.identificationType,cc.identificationNumber,cc.msisdn)),registrationDate, id from Consumer cc) c where c.serviceProvider.id in ?1 and c.registrationDate between ?2 and ?3 group by name")
    ...
    @Query(
            value =
                    "(" +
                            ...
                            "         GREATEST(COUNT(c.id) - :threshold, 0) AS value " +  // swap to CASE WHEN COUNT(c.id) > :threshold THEN 1 ELSE 0 END if preferred
                            "  FROM service_providers sp " +
                            "  LEFT JOIN consumers c ON c.service_provider_id = sp.id " +
                            "  WHERE (:providersIsNull = TRUE OR sp.name IN (:providers)) " +
                            "  GROUP BY sp.name " +
